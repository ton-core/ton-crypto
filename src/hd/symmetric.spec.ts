...
import { deriveSymmetricHardenedKey, deriveSymmetricPath, getSymmetricMasterKeyFromSeed } from "./symmetric";
...
describe('symmetric', () => {
    for (let tv of testVectors) {
        it('should match root key #' + tv.id, async () => {
            let res = await getSymmetricMasterKeyFromSeed(Buffer.from(tv.seed, 'hex'));
        ...
        for (let p of tv.paths) {
            it('should derive chain #' + tv.id + ' for path m/' + p.path.map((v) => `"${v}"`).join('/'), async () => {
                let state = await getSymmetricMasterKeyFromSeed(Buffer.from(tv.seed, 'hex'));
                let remainig = [...p.path];
        ...
        for (let p of tv.paths) {
            it('should derive full path #' + tv.id + ' for path m/' + p.path.map((v) => `"${v}"`).join('/'), async () => {
                let derived = await deriveSymmetricPath(Buffer.from(tv.seed, 'hex'), p.path);
