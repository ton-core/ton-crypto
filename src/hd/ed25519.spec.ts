    ...
    for (let v of testVectors) {
        for (let path of v.paths) {
            it('should process path for #' + v.id + ' m/' + path.path.map((v) => v + '\'').join('/'), async () => {
                ...
                // Derive
                let state = await getED25519MasterKeyFromSeed(Buffer.from(v.seed, 'hex'));
                ...
                // From seed
                let keypair = keyPairFromSeed(state.key);
    ...
    for (let v of testVectors) {
        for (let path of v.paths) {
            it('should derive full path for #' + v.id + ' m/' + path.path.map((v) => v + '\'').join('/'), async () => {
                let derived = await deriveEd25519Path(Buffer.from(v.seed, 'hex'), path.path);
