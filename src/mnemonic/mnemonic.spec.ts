...
describe('mnemonic', () => {
    ...
    for (let i = 0; i < testVectors.length; i++) {
        it('should match vector #' + i, async () => {
            let key = await mnemonicToPrivateKey(testVectors[i].mnemonics);
            let wk = await mnemonicToWalletKey(testVectors[i].mnemonics);
    ...
    it('should generate same keys for mnemonicToPrivateKey and mnemonicToWalletKey', async () => {
        for (let i = 0; i < 10; i++) {
            ...
            let key = await mnemonicToPrivateKey(k);
            let wk = await mnemonicToWalletKey(k);
